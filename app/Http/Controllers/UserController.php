...
class UserController extends Controller {
    ...
    public function __construct(UserApiController $API)
    {
        ...
        $this->middleware('auth', ['except' => ['index','single_video','all_categories' ,'category_videos' , 'sub_category_videos' , 'contact','trending', 'channel_videos', 'add_history', 'page_view', 'channel_list', 'watch_count', 'partialVideos', 'payment_mgmt_videos']]);
    }
    ...
    public function partialVideos(Request $request) {
        ...
        $channel = Channnel::find($request->channel_id);

        $view = View::make('user.videos.partial_videos')
                    ->with('videos', $videos)->with('channel', $channel)->render();
