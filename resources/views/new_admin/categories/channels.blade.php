							...
							<tbody>
								...
								@foreach($channels as $i => $channel)
								    ...
								    <tr>
		                            	...
		                            	<td><a target="_blank" href="{{route('admin.channels.subscribers',['id'=> $channel->id] )}}">{{$channel->get_channel_subscribers_count}}</a></td>

		                            	<td>{{Setting::get('currency')}} {{getAmountBasedChannel($channel->id)}}</td>

									    <td>
								      		@if($channel->is_approved == YES)
	            								...
	            								<li class="dropup">
									                ...
									                <ul class="dropdown-menu">
														...
														<li class="divider" role="presentation"></li>
									                  	...
									                  	@if($channel->is_approved == YES)
									                  		<li role="presentation"><a role="menuitem" tabindex="-1" href="{{route('admin.channels.status' , array('id' => $channel->id , 'status' =>0))}}">{{tr('decline')}}</a></li>
									                  	@else
									                  		<li role="presentation"><a role="menuitem" tabindex="-1" href="{{route('admin.channels.status' , array('id' => $channel->id , 'status' => 1))}}">{{tr('approve')}}</a></li>
									                  	@endif
