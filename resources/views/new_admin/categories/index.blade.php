						...
						<tbody>
							...
							@foreach($categories as $i => $category_details)
							    ...
							    <tr>
							      	...
							      	<td><a target="_blank" href="{{ route('admin.categories.view', ['category_id' => $category_details->id]) }}">{{ $category_details->name }}</a></td>

							      	<td><a target="_blank" href="{{ route('admin.categories.videos', ['category_id' => $category_details->id]) }}">{{ $category_details->get_videos_count }}</a></td>

							      	<td><img src="{{ $category_details->image }}" style="width: 25px;height: 35px"></td>
