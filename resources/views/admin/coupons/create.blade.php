				...
				<form action="{{route('admin.save.coupon')}}" method="POST" class="form-horizontal" role="form">
					...
					<div class="box-body">
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" name="coupon_code" min="1" max="10" class="form-control" value="{{old('coupon_code')}}"  required pattern="[A-Z0-9]+"  placeholder="{{tr('enter_coupon_code')}}" title="{{tr('validation')}}"><p class="help-block">{{tr('note')}} : {{tr('coupon_code_note')}}</p>
							</div>
						</div>
