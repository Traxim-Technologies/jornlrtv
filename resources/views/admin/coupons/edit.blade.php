				...
				<form action="{{route('admin.save.coupon')}}" method="POST" class="form-horizontal" role="form">
					...
					<div class="box-body">
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" name="coupon_code" min="5" max="10" class="form-control" pattern="[A-Z0-9]+" placeholder="{{tr('enter_coupon_code')}}" value="{{$edit_coupon->coupon_code ? $edit_coupon->coupon_code : old('coupon_code') }}"><p class="help-block">{{tr('note')}} : {{tr('coupon_code_note')}}</p>
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<textarea name="description" class="form-control" max="255">{{$edit_coupon->description}}</textarea>
							</div>
						</div>
					</div> 
